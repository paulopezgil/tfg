...
elements aleatoris amb valors a $(S, \rho)$,
tals que per a tot $n \geq 1$, $X^{(n)}$ i $Y^{(n)}$
estan definits en el mateix espai de probabilitats.
...
els espais de probabilitat on estan definits els elements aleatoris
$X^{(n)}$ i $Y^{(n)}$.
Sigui $(\Omega, \mathcal{F}, \P)$
un altre espai de probabilitat on està definit l'element aleatori $X$.
Sigui $f: S \to \mathbb{R}$ una funció contínua i acotada.
Per l'observació \ref{obs-convergencia-en-llei}, és suficient demostrar que
\begin{equation*}
...
$K_{\varepsilon} \subset S$ tal que
\begin{equation*}
...
\begin{equation*}
\P_n\left(\left|\rho(X^{(n)}, Y^{(n)}) - 0 \right| > \delta \right) < \frac{\varepsilon}{6M}, \quad \forall n \geq n_0.
\end{equation*}
