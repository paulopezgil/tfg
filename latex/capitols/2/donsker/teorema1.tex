...
\xlongrightarrow[n\to\infty]{\mathcal{L}}
...
\[
\big(X^{(n)}_s, X^{(n)}_t\big) \xlongrightarrow[n\to\infty]{\mathcal{L}} \big(B_s, B_t\big).
\]
...
\le \frac{\sigma^2}{\varepsilon^2\,\sigma^2 n}
= \frac{1}{\varepsilon^2 n} \xlongrightarrow{n\to\infty} 0.
\]
...
Fixem-nos en el primer factor (l'altre és idèntic substituïnt $s$ per $t-s$). Per una banda,
\[
\left|
...
\xlongrightarrow[n\to\infty]{P} 0.
\]
Per altra banda, pel Teorema central del límit,
\[
\frac{\sqrt{s}}{\sigma\sqrt{\lfloor ns\rfloor}} \sum_{j=1}^{\lfloor ns\rfloor} \xi_j \xlongrightarrow[n\to\infty]{\mathcal{L}} \mathcal{N}(0,s).
\]
Per tant, utilitzant la funció característica de la llei $\mathcal{N}(0,s)$ (cf. proposició \ref{prop:caracteristica-normal}) i el Teorema de continuïtat de Paul Lévy,
\[
...
\xlongrightarrow[n\to\infty]{\mathcal{L}} (B_s, B_t-B_s).
\]
...
\xlongrightarrow[n\to\infty]{\mathcal{L}} (B_s, B_t).
\end{equation}
