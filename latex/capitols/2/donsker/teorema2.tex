...
entenent $\left\{X^{(n)}\right\}_n$ i $X$ com variables aleatòries amb valors a $(C, \mathcal{C})$.

Per el teorema de Prohorov, com $\left\{ X^{(n)}\right\}_n$ és ajustada,
...
tal que $\mathcal{L}(Y^{(n)}) \xrightarrow{w} \P$, per a alguna $\P \in \mathcal{P}(C)$.

Suposem que una altra subsuccessió $\left\{Z^{(n)}\right\}_n$ indueix mesures
a $(C, \mathcal{C})$ que convergeixen feblement a una mesura $\Q \in \mathcal{P}(C)$.
Aleshores, $\P$ i $\Q$ han de tenir les mateixes distribucions a dimensió finita.
És a dir,
\begin{equation*}
\P\left[ w \in \mathcal{C} : (w(t_1), \ldots, w(t_d)) \in A\right]
=
\Q\left[ w \in \mathcal{C} : (w(t_1), \ldots, w(t_d)) \in A\right]
\end{equation*}
...
Aleshores, existeix una funció $f: \mathcal{C} \rightarrow \mathbb{R}$
