...
Siguin $\{X^{(n)}\}_n$ les trajectòries del passeig
aleatori convenientment normalitzades com a l'equació \eqref{interpolacio-lineal}.
Aleshores, per a tot $s < t < \infty$,
\[
...
$Im(\xi_k) = \{-1,1\}$, $\mathbb{E}[\xi_k] = 0$ i $\operatorname{Var}(\xi_k) = 1$ per a tot $k \geq 1$.
Per la hipòtesi independència i tenint en compte que
$\xi_1, \xi_2, \xi_3, \xi_4$ estan ordenades,
obtenim que
\[
...
Substituïnt a l'expressió \eqref{eq:moment-quart-2}, obtenim que
\[
\mathbb{E}\bigl[\theta(x_1) \cdots \theta(x_4) \mathbf{1}_{\{x_1 \leq x_2 \leq x_3 \leq x_4\}}\bigr] =
...
Aplicant això a l'equació \eqref{eq:moment-quart}, obtenim la següent expressió:
\begin{align*}
&\mathbb{E}\bigl[(X^{(n)}_t - X^{(n)}_s)^4\bigr] \nonumber \\
...
\[
\mathbb{E}\bigl[(X^{(n)}_t - X^{(n)}_s)^4\bigr]
\leq \frac{24}{n^2} \left( \sum_{k=1}^\infty \int_{ns}^{nt} \int_{ns}^{nt}
\mathbf{1}_{[k-1,k)}(x_1) \mathbf{1}_{[k-1,k)}(x_2) \mathbf{1}_{\{x_1 \leq x_2\}}\,dx_1 dx_2 \right)^2.
\]

Ara fem el canvi de variables $y_1 = x_1/n$ i $y_2 = x_2/n$:
\[
\mathbb{E}\bigl[(X^{(n)}_t - X^{(n)}_s)^4\bigr] \leq
...
\begin{align*}
\mathbb{E}\bigl[(X^{(n)}_t - X^{(n)}_s)^4\bigr] 
&\leq 24 n^2 \left( \int_s^t \int_{\max(y_2 - \frac{1}{n}, s)}^{y_2} dy_1\,dy_2 \right)^2 \\
