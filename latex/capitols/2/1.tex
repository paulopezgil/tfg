...
Les següents definicions són essencials per a comprendre la convergència de
processos estocàstics i la seva aplicació en l'aproximació del moviment Brownià.
...
\begin{defi}
Sigui $(S,\rho)$ un espai mètric. Considerem en aquest espai mètric la $\sigma$-àlgebra de Borel $\mathcal{B}(S)$.
...
\begin{equation*}
PX^{-1}(B) = \mathbb{P}(X \in B) = \mathbb{P}(X^{-1}(B)) \quad \forall B \in \mathcal{S}.
\end{equation*}
...
i considerem en cada un d'ells una variable aleatòria $X_n$ amb valors a un mateix espai mètric $(S, \rho)$.
Sigui $\left( \Omega, \mathcal{F}, \mathbb{P} \right)$ un altre espai de probabilitat
i $X$ una variable aleatòria en aquest espai amb valors a $(S, \rho)$.
...
\begin{equation*}
\mathbb{E}[f(X_n)] = \int_{\Omega_n} f(X_n(\omega_n)) d\mathbb{P}_n(\omega_n) = \int_S f(s) dPX_n^{-1}(s),
\end{equation*}
i de manera anàloga,
\begin{equation*}
